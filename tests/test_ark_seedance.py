...
def test_passes_through_ark_trusted_material_asset_uri():
    model = ArkSeedanceVideoModel(
        {
            "api_key": "test-key",
            "base_url": "https://ark.cn-beijing.volces.com/api/v3",
    ...
    body = model._build_request_body(
        prompt="保持虚拟角色一致",
        generation_mode="r2v",
        ref_image_urls=["asset://asset-20260820abc123"],
    )

    assert body["content"][1] == {
        "type": "image_url",
        "image_url": {"url": "asset://asset-20260820abc123"},
...
def test_pipeline_prefers_bound_ark_asset_uri_for_seedance_r2v(monkeypatch):
    preview_url = "assets/characters/ref.png"
    variant = ImageVariant(
        id="variant-1",
        url=preview_url,
        provider_asset_ids={"volcengine_ark": "asset-20260820abc123"},
    )
    character = Character(
        id="character-1",
        name="Virtual Character",
        description="An original synthetic character",
    ...
    class FakeArkModel:
        ...
        def generate(self, **kwargs):
            calls["generate"] = kwargs
            kwargs["on_provider_ids"]("volcengine-ark", "cgt-r2v", "req-r2v")
            return kwargs["output_path"], 0.0
    ...
    pipeline._kling_model = None
    pipeline._vidu_model = None
    pipeline.video_generator = SimpleNamespace(model=SimpleNamespace())
    pipeline.get_script = lambda script_id: pipeline.scripts.get(script_id)

    pipeline.process_video_task("script-1", "task-seedance-r2v")

    assert task.status == "completed"
    assert calls["generate"]["ref_image_urls"] == [
        "asset://asset-20260820abc123"
    ]
    assert task.reference_image_urls == [preview_url]
