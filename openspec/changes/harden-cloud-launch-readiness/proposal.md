...
- Introduce controlled registration modes: `disabled`, `invite_only`, explicitly unverified `open`, and a reserved future `verified_open`. `open` is allowed only through an explicit configuration decision and keeps the phone unverified; add audited, expiring, single-use, phone-bound invitations for controlled beta use.
...
- `controlled-beta-registration`: Disabled, invitation-only, explicitly unverified open, and future verified phone-registration modes with auditable policy changes.
...
- Staging and rollout evidence tooling. The tools do not autonomously call a paid provider, expand production registration, or remove compatibility routing without explicit operator confirmation and external evidence.
